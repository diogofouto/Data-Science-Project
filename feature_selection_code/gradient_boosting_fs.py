...
('air_quality_train_smote_no_low_variance_vars_no_correlated_vars_0.9', 'air_quality_test_no_low_variance_vars_no_correlated_vars_0.9', 'No_correlated_vars_0.9', AQ_TARGET)]:
        ...
        best = gradient_boosting_study(X_train, y_train, X_test, y_test, [5], filetag)
    ...
    bar_chart(lst_variables, lst_recall_train, ax=axs[0], title='Air Quality Train set', xlabel='corr threshold', ylabel='recall', rotation=45)
    bar_chart(lst_variables, lst_recall_test, ax=axs[1], title='Air Quality Test set', xlabel='corr threshold', ylabel='recall', rotation=45)
    plt.savefig('images/lab7/gradient_boosting/air_quality_correlation_gb_study', dpi=300, bbox_inches="tight")
    plt.clf() # cleanup
